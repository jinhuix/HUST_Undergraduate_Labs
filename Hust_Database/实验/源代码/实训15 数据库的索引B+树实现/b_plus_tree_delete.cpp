...
bool BPLUSTREE_TYPE::CoalesceOrRedistribute(N *node) {
  ...
  // 如果两个结点的元素可以合并，则调用Coalesce()进行合并
  ...
  // 如果元素过多，无法合并，则调用Redistribute()进行结点元素的调整
...
bool BPLUSTREE_TYPE::AdjustRoot(BPlusTreePage *old_root_node) {
    ...
    // 根结点可能是叶结点，也可能是内部结点，分开处理
    // 当根结点中最后一个元素被删除，此时B+树为空
    // 调用UpdateRootPageId()更新根结点page_id
